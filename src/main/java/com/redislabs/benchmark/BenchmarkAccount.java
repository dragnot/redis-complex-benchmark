...
public class BenchmarkAccount implements Runnable {
     ...
     */
    public BenchmarkAccount(RedisClient redisClient, int topBusiness, int[] accounts)
    {
    ...
    public void run() {
        ...
        logger.info("Starting on prefix " + accounts[i]);
        //continue to run until the flag is set to false
        while(continueRun) {

            stopwatch.start();
            Long results =  unionBusiness(BenchmarkUtils.getBuisnessName(  accounts[i] ));
            if ( results == -1 )
